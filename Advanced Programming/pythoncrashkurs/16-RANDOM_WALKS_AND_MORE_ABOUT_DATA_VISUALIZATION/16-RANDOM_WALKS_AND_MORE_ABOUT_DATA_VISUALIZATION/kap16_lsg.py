...
###############################################################################################
                ...
                ###   Lösungen zu Kap. 16 RANDOM WALKS UND MEHR ÜBER DATENVISUALISIERUNG    ###
...
#Da das importieren von Definitionen aus kap16.py immer auch die Ausführung der gesamten 
#Datei bedeutet, z.B. 
#             from kap16 import Location
#etc., kopiere ich das Zeug, das ich brauche, lieber einfach hierher 
...
#Der folgende Textabschnitt ist ebenfalls einfach aus kap16.py kopiert worden,
#da er die zu lösende Aufgabe näher erläutert:
#
#       Durch drunk_test(...) wird Folgendes gedruckt:
...
#zur Lösung:
#Der Code 16.3.2 ist sehr ähnlich zu unserer gesuchten Lösung - davon kupfern wir einfach ab,
#bzw. modifizieren für unsere Zwecke...
...
def sim_all_plot(d_class, walk_lengths, num_trials):       #modifiziert: nur eine Drunk-Klasse wird hier berücksichtigt
    means = sim_drunk(num_trials, d_class, walk_lengths)
    plt.plot(walk_lengths, means, 'm-', label = d_class.__name__)
    ###NEU
    import math
    plt.plot(walk_lengths, list(map(math.sqrt, walk_lengths)), 'r:', label = 'sqrt(steps)')   #hier erzeugen wir die Datenpunkte für die Wurzeln der walk_lengths
    ###NEU
...
sim_all_plot(Usual_drunk, (10, 100, 1000, 10000, 100000), 100)
#(siehe Abb-Lsg-Aufg-16-2-a.PNG)
